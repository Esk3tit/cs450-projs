...
const char *WINDOWTITLE = "Project #5 -- Khai Phan";
...
bool	VertexFrozen;			// Freeze vertex shader anim
bool	FragmentFrozen;			// Freeze fragment shader anim
...
Display( )
{
	...
	// Turn on shaders and then off after we draw what we need
	Pattern->Use( );
	...
	if ( !VertexFrozen )
	{
		X = -sin( 360. * Time * 0.25 );
		Y = -cos( 360. * Time * 0.25 );
		Z = 3 * -tan( 360. * Time * 0.25 );
	}
	Pattern->SetUniformVariable( "uX", X );
	Pattern->SetUniformVariable( "uY", Y );
	...
	Pattern->SetUniformVariable( "uSpecularColor", specularColor );
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'b':
			VertexFrozen = false;
			FragmentFrozen = false;

			break;

		case 'f':

			if ( !VertexFrozen || !FragmentFrozen )
			{
				VertexFrozen = true;
				FragmentFrozen = true;
			}
		...
		case 'F':
			FragmentFrozen = !FragmentFrozen;
			break;

		case 'V':
			VertexFrozen = !VertexFrozen;
			break;
