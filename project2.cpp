...
const char *WINDOWTITLE = "Project #2 -- Khai Phan";
...
GLuint	PolygonCessnaList;		// list to hold polygon version of Cessna
float	Scale;					// scaling factor
int		ShadowsOn;				// != 0 means to turn shadows on
float	Time;					// keep track of time for animations
GLuint	WireframeCessnaList;    // list to hold wireframe version of Cessna
...
GLuint	Building1List;			// list to hold a building
GLuint	Building2List;			// list to hold another building
GLuint	PropellerList;			// list to hold propeller
...
Display( )
{
	...
	// Only allow outside look at and rotation and scaling if outside view
	// otherwise use hardcoded inside view of Cessna
	...
	// draw the polygon Cessna object by calling up its display list:

	glCallList( PolygonCessnaList );
	...
	// First prop, front prop.
	// Radius is already 1, so to get radius of 5 just scale everything by 5
	glPushMatrix( );
	...
	// Second prop, left wing top prop.
	// Radius is already 1, so to get radius of 3 just scale everything by 3
	glPushMatrix( );

	glTranslatef( -10., 3., 0 );

	// Rotate for animation
	glRotatef( 2 * -Time * 360., 0., 1., 0. );

	// Rotate to orient the propeller properly
	glRotatef( -90., 1., 0., 0. );
	glScalef( 3., 3., 3. );
	...
	// Third prop, right wing top prop.
	// Radius is already 1, so to get radius of 3 just scale everything by 3
	glPushMatrix( );
...
InitLists( )
{
	...
	// Cessna display lists
	PolygonCessnaList = glGenLists( 1 );
	glNewList( PolygonCessnaList, GL_COMPILE );
		PolygonCessna( );
	glEndList( );

	WireframeCessnaList = glGenLists( 1 );
	glNewList( WireframeCessnaList, GL_COMPILE );
		WireframeCessna( );
	glEndList( );
	...
	glNewList( Building1List, GL_COMPILE );
		glPushMatrix( );
		glRotatef( -15., 1., 0., 0. );
		LoadObjFile( "Residential Buildings 009.obj" );
		glPopMatrix( );
	glEndList( );
	...
	glNewList( Building2List, GL_COMPILE );
		glPushMatrix( );
		glRotatef( -15., 1., 0., 0. );
		LoadObjFile( "Residential Buildings 010.obj" );
		glPopMatrix( );
	glEndList( );
