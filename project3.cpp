...
const char *WINDOWTITLE = "Project #3 -- Khai Phan";
...
int		WhichTexture;			// determine texture mode (off, normal, distorted)
...
void	OsuSphere( float, int, int, float, float );
void	Reset( );
void	Resize( int, int );
void	Visibility( int );
...
Display( )
{
	...
	// Dynamically distort s and t coords so we can't use Display list since we can't distort
	// display list stuff (white sphere)
	...
	if ( WhichTexture != DISTORTED )
		OsuSphere( 3, 50, 50, 0, 0 );
	else
		OsuSphere( 3, 50, 50, Time, Time );
...
DoTextureMenu( int id )
{
	/*switch( id )
	{
		...
		default:
			fprintf( stderr, "Don't know what to do with Texture Mode ID %d\n", id );;
	}*/
...
InitGraphics( )
{
	...
	// Texture initialization (binding and init state of texture)
	int width, height;
	Texture = BmpToTexture( "xi.bmp", &width, &height );
	if ( Texture == NULL )
...
void
OsuSphere( float radius, int slices, int stacks, float distortedS, float distortedT )
{
	// set the globals:
	...
	for ( int ilat = 0; ilat < SphNumLats; ilat++ )
	{
			...
			// ilng=SphNumLngs-1, lng=+M_PI are the same meridian
		{
			...
			{
				//p->s = (( lng + M_PI ) / ( 2. * M_PI )) * cosf( 0.25 * Time * 90. );
				p->s = ( ( lng + M_PI ) / ( 2. * M_PI ) * sinf( 360. * p->t ) * Time);
				p->t = (( lat + M_PI / 2. ) / M_PI);
			}
		}
	}
